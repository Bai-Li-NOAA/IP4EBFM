...
library(stocksmart) # remotes::install_github("NOAA-EDAB/stocksmart")
...
for (scenario_id in seq_along(add_environmental_effects_vec)){
  ...
  # Load data-poor RData
  load(here::here("data", "data_poor", ewe_scenario_name,
                  paste0("dbsra_projection_output3_", terminal_year, scenario_filename, ".RData")))
  temp <- data.frame(
    scenario = paste0("S", scenario_id),
    model = "Data-poor EM",
    msy = projection_output3[[1]]$MSYstore
  )

  if (scenario_id == 1){
    msy_data <- temp
  } else {
    msy_data <- rbind(msy_data, temp)
  }
...
ggplot(lm_data[which(!lm_data$Variable == "IV"), ],
...
ggsave(file.path(figure_path, paste0(terminal_year, scenario_filename, "_linear_regression_less_rows.jpeg")),
       width = 6, height = 4, dpi = 1000, units = "in", device='png')
...
ggplot(soi_data[soi_data$projection_year_id == 2013 & !(soi_data$variable %in% "IV"), ], aes(x = year, y = value, color = Model)) +
  geom_line(alpha = 0.8, size=0.4, aes(linetype = Model)) +
  geom_point(aes(shape = Model), size = 0.2) +
...
ggsave(file.path(figure_path, paste0(terminal_year, scenario_filename, "_soi_less_rows.jpeg")),
       width = 4.5, height = 5, dpi = 1000, units = "in", device='png')
...
ggsave(file.path(figure_path, paste0(terminal_year, scenario_filename, "_bratio_line.jpeg")),
       width = 5, height = 4, dpi = 1000, units = "in", device='png')

# MSY ------------------------------------------------------------------
load(here::here("data", "ewe", "ewe_reference_points.RData"))
msy_data <- rbind(msy_data,
                  c("S1", "OM", reference_points$msy["Compensation MSY s1"]),
                  c("S2", "OM", reference_points$msy["Compensation msy s2"]),
                  c("S3", "OM", reference_points$msy["Compensation msy s2"]))
msy_data$model <- factor(msy_data$model, levels = c(
  "OM",
  "Data-poor EM",
  "Data-moderate EM",
  "Data-rich EM"))
msy_data$msy <- as.numeric(msy_data$msy)

ggplot(msy_data, aes(x = scenario, y = msy, color = model)) +
...
labs(
  ...
  guides(color=guide_legend(title="Model"))
ggsave(file.path(figure_path, paste0(terminal_year, scenario_filename, "_msy_line.jpeg")),
       width = 5, height = 4, dpi = 1000, units = "in", device='png')

ggplot(msy_data[!(msy_data$model == "Data-poor EM"),], aes(x = scenario, y = msy, color = model)) +
  geom_boxplot(outlier.shape = NA) +
  labs(
    x = "Scenario",
    y = "MSY (mt)"
  ...
  guides(color=guide_legend(title="Model"))
ggsave(file.path(figure_path, paste0(terminal_year, scenario_filename, "_msy_line_without_data_poor.jpeg")),
       width = 5, height = 4, dpi = 1000, units = "in", device='png')
...
levels(fmsy_data$variable) <- c("EM FMSY", paste0("Fadj-I", c(1:7, "V", 8:9)))
...
# boxplot
temp_data <- fmsy_data[!(fmsy_data$variable == "Fadj-IV"),]

ggplot(temp_data, aes(x = variable, y = value, color = projection_year_id)) +
  geom_boxplot(outlier.size = 0.2) +
  facet_grid(model~scenario, scales = "free") +
  labs(
    x = "",
    y = "Fishing Mortality Rate (F)"
  ) +
  theme_bw() +
  theme(
    axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
  ...
  guides(color=guide_legend(title="Projection Year"))
...
medianf_data$variable <- factor(medianf_data$variable, levels = c("Default F", "amo", "pdsi", "bassB", "meanage",
                                                            ...
                                                            "menhadenV",
                                                            "menhadenE", "menhadenCPUE"))
levels(medianf_data$variable) <- c("EM FMSY", paste0("Fadj-I", c(1:7, "V", 8:9)))
...
combine_figures <- function(lm_data, soi_data, bratio_data, projection_data,
  ...
  indicator_color <- hue_pal()(12)
  names(indicator_color) <- c("OM", "FMSY-EM", paste0("Fadj-I", 1:9), "Ensemble")

  indicator_shape <- 0:11
  names(indicator_shape) <- c("OM", "FMSY-EM", paste0("Fadj-I", 1:9), "Ensemble")
  ...
  lm_figure <- ggplot(
    ...
    guides(color=guide_legend(title="Model"))
  ggsave(paste0(figure_path, "_lm.jpeg"),
         width = 5, height = 4, dpi = 1000, units = "in", device='jpeg')
  ggsave(paste0(figure_path, "_lm.pdf"),
         width = 5, height = 4, dpi = 1000, units = "in", device='pdf')
  ...
  soi_figure <- ggplot(
    ...
    geom_point(data_subset[which(data_subset$year == tail(model_year, n = 1)), ],
               mapping = aes(x = year, y = value, pch = Model), size = 0.2, alpha = 0.5
    ) +
    scale_colour_manual(values = model_color[c("OM", em_name)]) +
    ...
    guides(color=guide_legend(title="Model"))
  ggsave(paste0(figure_path, "_soi.jpeg"),
         width = 5, height = 4, dpi = 1000, units = "in", device='jpeg')
  ggsave(paste0(figure_path, "_soi.pdf"),
         width = 5, height = 4, dpi = 1000, units = "in", device='pdf')

  # Bratio
  if (em_name == "Data-rich EM"){
    bratio_ylabel <- bquote(SB[2012]/SB[MSY])
  } else {
  ...
  ggsave(paste0(figure_path, "_bratio.jpeg"),
         width = 5, height = 4, dpi = 1000, units = "in", device='jpeg')
  ggsave(paste0(figure_path, "_bratio.pdf"),
         width = 5, height = 4, dpi = 1000, units = "in", device='pdf')
  ...
  f_projection_data <- data_subset[which(data_subset$Year_type == "Projection" &
                                           data_subset$variable == "F_apical" &
                                           !(data_subset$Model %in% c(em_name)) &
                                           !(data_subset$Data_type %in% c("min", "max"))), ]


  f_avg <- f_projection_data %>%
    filter(variable == "F_apical" & Model %in% paste0("Fadj-", indicator_id)) %>%
    group_by(Year) %>%
    summarise(Ensemble = mean(value, na.rm = TRUE))

  # Create the f_ensemble dataframe
  f_ensemble <- f_avg %>%
    mutate(Model = "Ensemble")

  f_projection_figure <- ggplot() +
    geom_line(
      f_projection_data,
      mapping = aes(
        x = Year, y = value,
    ...
    scale_colour_manual(
      "",
      values = indicator_color[c("OM", "FMSY-EM", paste0("Fadj-", indicator_id), "Ensemble")]
    ) +
    scale_shape_manual(
      "",
      values = indicator_shape[c("OM", "FMSY-EM", paste0("Fadj-", indicator_id), "Ensemble")]
    ) +
    labs(
      color = "",
      shape = "",
  ...
  ggsave(paste0(figure_path, "_Fprojection.jpeg"),
         width = 5, height = 4, dpi = 1000, units = "in", device='jpeg')
  ggsave(paste0(figure_path, "_Fprojection.pdf"),
         width = 5, height = 4, dpi = 1000, units = "in", device='pdf')
  ...
  if (em_name == "Data-moderate EM") {
    data_subset <- data_subset[-which(data_subset$variable == "F_average"), ]
  }
  ...
  if (nrow(ensemble_data) != 0) {
    ...
    ensemble_projection$Model <- "Ensemble"
  ...
  projection_figure_data <- data_subset[which(!(data_subset$variable == "F_apical")), ]

  projection_figure <- ggplot() +
    geom_line(
      percentage_change[which(!(percentage_change$Model == em_name) &
                                !(percentage_change$variable == "F_apical") &
                                !(percentage_change$Model %in% paste0("Fadj-", indicator_id))), ],
      mapping = aes(
    ...
    scale_colour_manual("",
                        values = indicator_color[c("OM", "FMSY-EM", "Ensemble")]
    ) +
    scale_shape_manual("",
                       values = indicator_shape[c("OM", "FMSY-EM", "Ensemble")]
    ) +
    labs(
      color = "",
      shape = "",
  ...
  ggsave(paste0(figure_path, "_projections.jpeg"),
         width = 5, height = 3, dpi = 1000, units = "in", device='jpeg')

  ggsave(paste0(figure_path, "_projections.pdf"),
         width = 5, height = 3, dpi = 1000, units = "in", device='pdf')
  # Combine figures
  # Without lm_figure
  ...
  ggsave(paste0(figure_path, "_combined_figures.jpeg"),
         width = 5, height = 5, dpi = 1000, units = "in", device='jpeg')
  ggsave(paste0(figure_path, "_combined_figures.pdf"),
         width = 5, height = 5, dpi = 1000, units = "in", device='pdf')
...
combine_figures(
  ...
  model_year = model_year,
  projection_year = projection_year,
  figure_path = file.path(figure_path, paste0(terminal_year, scenario_filename, "_data_rich_S2_combined")),
  scale_projection = TRUE
)
...
# F adjustment conceptual figure ------------------------------------------

data("stockAssessmentSummary")

fmsy_low <- stockAssessmentSummary$Fmsy[which(stockAssessmentSummary$`B/Bmsy`<=0.5)]
fmsy_low_median <- median(fmsy_low, na.rm = TRUE)


fmsy_medium <- stockAssessmentSummary$Fmsy[which(stockAssessmentSummary$`B/Bmsy`> 0.5 &
                                                   stockAssessmentSummary$`B/Bmsy`<=1)]
fmsy_medium_median <- median(fmsy_medium, na.rm = TRUE)

fmsy_high <- stockAssessmentSummary$Fmsy[which(stockAssessmentSummary$`B/Bmsy`> 1)]
fmsy_high_median <- median(fmsy_high, na.rm = TRUE)

median_fmsy <- c(0.3, 0.5, 0.7)
...
for (i in 1:3){
  FMSY[[i]] <- data.frame(FMSY = rnorm(sample_size, median_fmsy[i], 0.02))
}
# FMSY <- data.frame(FMSY = rnorm(sample_size, 0.8, 0.05))
# FMSY_lower <- min(FMSY$FMSY)
# FMSY_upper <- max(FMSY$FMSY)

SOI_level <- seq(0, 1, by = 0.25)

Bratio_mean <- seq(0.1, 1.5, by=0.1)
sample_min <- 0
Bratio <- Bratio_level <-
  df_add_fmsy <- list()
for (i in seq_along(Bratio_mean)){
  Bratio[[i]] <- rnorm(sample_size, Bratio_mean[i], 0.1)
  Bratio[[i]][Bratio[[i]] < 0] <- 0
  Bratio_level[[i]] <- rep(Bratio_mean[i], sample_size)
  if (Bratio_mean[i] <= 0.5) FMSY_temp <- FMSY[[1]]$FMSY
  if (Bratio_mean[i] > 0.5 & Bratio_mean[i] <= 1.0) FMSY_temp <- FMSY[[2]]$FMSY
  if (Bratio_mean[i] > 1) FMSY_temp <- FMSY[[3]]$FMSY

  df_add_fmsy[[i]] <- expand.grid(Bratio = Bratio[[i]], FMSY = FMSY_temp)
  df_add_fmsy[[i]]$SOI_level <- "All levels"
}

# Create an empty list to store data frames
df_list <- list()

# Loop over SOI levels to calculate Fadj for each level and store in list
for (SOI in SOI_level) {
  df_temp_list <- list()
  for (i in seq_along(Bratio_mean)) {
    ...
    Fadj <- ifelse(Bratio[[i]] > 1,
                   min(FMSY_temp) + SOI * (max(FMSY_temp) - min(FMSY_temp)),
                   ifelse(Bratio[[i]] > 0.5,
                          SOI * FMSY_temp * Bratio[[i]],
                          0
                   )
    )

    # Create a data frame for the current SOI level
    df_temp <- data.frame(Bratio = Bratio[[i]], Bratio_level = Bratio_level[[i]],
                          Fadj = Fadj,
                          SOI_level = as.factor(SOI))

    df_temp_list[[i]] <- df_temp
  }

  # Combine data frames for the current SOI level
  df_list[[as.character(SOI)]] <- bind_rows(df_temp_list, .id = "Bratio_mean")
}

# Combine all data frames into one
df_combined <- bind_rows(df_list, .id = "SOI_level")
df_combined$SOI_level <- factor(df_combined$SOI_level)

# Combine all FMSY data into one data frame
df_add_fmsy_combined <- bind_rows(df_add_fmsy, .id = "Bratio_mean")

# Determine y-axis range for Fadj
y_range <- range(c(df_combined$Fadj, df_add_fmsy_combined$FMSY), na.rm = TRUE)

color_palette <- brewer.pal(n = length(SOI_level), name = "Set2")

# Calculate Q1 and Q3 for Fadj
df_summary <- df_combined %>%
  group_by(SOI_level, Bratio_level) %>%
  summarize(
    Q1 = quantile(Fadj, 0.25, na.rm = TRUE),
    Q3 = quantile(Fadj, 0.75, na.rm = TRUE),
    Fadj_median = median(Fadj, na.rm = TRUE)
  ) %>%
  ungroup()
...
ggplot() +
  ...
  scale_color_manual(
    name = "Status of Indicator",
    values = c("0" = color_palette[1],
               "0.25" = color_palette[2],
               "0.5" = color_palette[3],
               "0.75" = color_palette[4],
               "1" = color_palette[5],
               "FMSY" = "black"),
    labels = c(as.character(SOI_level), bquote(EM~F[MSY])),
  ...
  scale_fill_manual(
    ...
    values = c("0" = color_palette[1],
               ...
               "FMSY" = "grey"),
    labels = c(as.character(SOI_level), bquote(EM~F[MSY])),
    guide = guide_legend(nrow = 1, override.aes = list(shape = NA, linetype = "blank"))
  ) +
  ...
  scale_x_continuous(breaks = Bratio_mean) +
  ylim(range(df_combined$Fadj, na.rm = TRUE)) +
  guides(color = "none")

ggsave(file.path(figure_path, "Fadj_conceptual_ribbon.jpeg"),
