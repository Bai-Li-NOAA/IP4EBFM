...
evaluation_table <- data.frame(
  ...
  B2017_K = tail(biomass_ewe, n=1)/om_K, 
  B2017_BMSY = mean(tail(biomass_ewe, n=1)/om_bmsy),
  Average_Biomass = mean(tail(biomass_ewe, n=length(projection_year))),
  Bonanza_Period = sum((tail(biomass_ewe, n=length(projection_year))/om_K)>0.8),
  Collapse_Period = sum((tail(biomass_ewe, n=length(projection_year))/om_K)<0.2)
)
...
radarchart(evaluation_table,
           ...
           paxislabels = round(evaluation_table["Max",],2), palcex = 2, seg = 5)
