...
ss_case0_input_noEffort$jagsdata$r.pr <- c(0.2, 0.5)
# ss_case0_input_noEffort$jagsdata$r.pr <- c(0.6, 0.1)
psi_init <- biomass_ewe[1]/k_init
ss_case0_input_noEffort$jagsdata$psi.pr <- c(psi_init, 0.5)
...
# psi_init <- biomass_ewe[1]/k_init
# ss_case0_input$jagsdata$psi.pr <- c(psi_init, 0.1)

k_init <- B0_F0_menhaden
ss_case0_input$jagsdata$K.pr <- c(k_init, 0.5)
ss_case0_input$jagsdata$r.pr <- c(0.2, 0.5)
# ss_case0_input$jagsdata$r.pr <- c(0.6, 0.1)
psi_init <- biomass_ewe[1]/k_init
ss_case0_input$jagsdata$psi.pr <- c(psi_init, 0.5)
...
ss_case0$estimates
write.csv(ss_case0$estimates, file.path(output_dir, "case0_estimates.csv"))
...
if (!dir.exists(figure_dir)) dir.create(figure_dir)
JABBA::jabba_plots(jabba = ss_case0, output.dir = figure_dir)
# plot with CIs (80% for projections)
JABBA::jbplot_prj(ss_case0, type = "BBmsy", output.dir = figure_dir)
JABBA::jbplot_prj(ss_case0, type = "BB0", output.dir = figure_dir)
JABBA::jbplot_prj(ss_case0, type = "FFmsy", output.dir = figure_dir)
...
cpue <- sa_data$fishery$om_total_catch_biomass/effort
...
# projection_f[, paste0("f", sa_data$biodata$ages)] <- NA
...
for (i in 1:nrow(projection_f)) {
    ...
    projection_f[i, paste0("fmsy0.25_", sa_data$biodata$ages)] <- projection_f$value[i] * fishery_sel*0.25
}

write.csv(fmsy_data_melt, here::here("data", "data_moderate", "fmsy_data.csv"))
write.csv(projection_f, here::here("data", "data_moderate", "fmsy_median_data.csv"))
...
cases <- c("ecosim_data_moderate_jabba_highF",
           "ecosim_data_moderate_amo_highF",
           "ecosim_data_moderate_pdsi_highF",
           "ecosim_data_moderate_bassB_highF",
           "ecosim_data_moderate_effort_highF")
...
cases <- c("ecosim_data_moderate_jabba_mediumF",
...
cases <- c("ecosim_data_moderate_jabba_lowF",
           "ecosim_data_moderate_amo_lowF",
           "ecosim_data_moderate_pdsi_lowF",
           "ecosim_data_moderate_bassB_lowF",
           "ecosim_data_moderate_effort_lowF")
