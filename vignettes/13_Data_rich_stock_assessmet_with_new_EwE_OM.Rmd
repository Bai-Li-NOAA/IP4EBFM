...
# MCMC run
mcmc_dir <- file.path(dirname(here::here()), "OM2_mcmc")
mcmc_output <- SSgetMCMC(mcmc_dir)
fmsy_mcmc <- mcmc_output$annF_MSY
bratio_2012_mcmc <- mcmc_output$Bratio_2012

# mcmc_dir <- here::here("data", "data_rich", "OM2_mcmc")
# generate_ss3(
#   file_path = mcmc_dir,
#   r0 = 12, steepness = 0.55, sigmar = 0.5,
#   projection_f = fmsy, projection_catch = NULL,
...
ylim <- range(ssb_ewe[time_id], ss3list$timeseries$SpawnBio)
...
lines(projection_year,
      ss3list$timeseries$SpawnBio[projection_year_id],
      lty = 2
)
legend("topleft",
...
# cor(ssb_ewe[time_id][1:length(model_year)], ss3list$timeseries$SpawnBio[model_year_id])
...
# cor(abundance_ewe[1:length(model_year)], ss3list$timeseries$`SmryNum_SX:1_GP:1`[model_year_id] * 1000)
...
# cor(recruit_ewe[1:length(model_year)], recruit_ss3$recruit[1:length(model_year)])
...
# cor(mortality_ewe[1:length(model_year)], ss3list$timeseries$`F:_1`[model_year_id])
...
for (projection_year_id in 1:length(projection_year)) {
  ...
  if (projection_year_id == length(projection_year)) {
    ...
    plot(amo$scaled_value[index_lag_id], menhaden_b[biomass_lag_id],
      xlab = "AMO values",
      ylab = "SB of menhaden-like species"
    )
    abline(amo_lm)
...
ggplot(fmsy_data_melt_median, aes(x = projection_year_id, y = value, color = variable)) +
  geom_line() +
  labs(
    title = "Median FMSY from SS3 and adjusted F",
    x = "",
    y = "FMSY"
  ) +
  theme_bw()
...
for (i in 1:nrow(fmsy_vec)) {
  ...
  local({
    working_dir <- projection_file_path
    shell(
        paste0(
        "cd ", working_dir,
        " & ss_win "
...
projection_f <- fmsy_data_melt_median
...
for (i in 1:nrow(projection_f)) {
    ...
    projection_f[i, paste0("fmsy0.5_", sa_data$biodata$ages)] <- projection_f$value[i] * fishery_sel*0.5
    projection_f[i, paste0("fmsy1.5_", sa_data$biodata$ages)] <- projection_f$value[i] * fishery_sel*1.5
}

write.csv(fmsy_data_melt, here::here("data", "data_rich", "fmsy_data.csv"))
write.csv(projection_f, here::here("data", "data_rich", "fmsy_median_data.csv"))
...
cases <- c("ecosim_data_rich_ss3",
           "ecosim_data_rich_amo",
           "ecosim_data_rich_pdsi",
           "ecosim_data_rich_bassB",
           "ecosim_data_rich_effort",
           "ecosim_data_rich_meanage")
...
for (i in 1:case_num){
  colors_border <- colors[c(1, 1+i, 7, 7+i)]
  radarchart(evaluation_table[c(1:3, 3+i, 9, 9+i), ],
             seg=5,
...
colors_border <- colors[c(7:12)]
radarchart(evaluation_table[c(1:2, 9:14),],
           seg=5,
           pcol=colors_border, plwd=2 , plty=1,
...
- Project biomass based on $0.5\times F_{MSY}$
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
# Re-run SS3 with Fadj

projection_file_path <- here::here("data", "data_rich", "OM2_projections")
ss3_projection <- list()
fmsy_vec <- aggregate(0.5*value ~ variable, fmsy_data_melt_median, median)
...
for (i in 1:nrow(fmsy_vec)) {
  ...
  file_path = projection_file_path,
  r0 = 12, steepness = 0.55, sigmar = 0.5,
  projection_f = fmsy_vec$`0.5 * value`[i], projection_catch = NULL,
  sa_data = sa_data, model_year = model_year,
  projection_year = projection_year,
  use_depletion = FALSE, depletion_ratio = NULL,
  initial_equilibrium_catch = TRUE, 
...
cases <- c("ecosim_data_rich_ss3_lowF",
           "ecosim_data_rich_amo_lowF",
           "ecosim_data_rich_pdsi_lowF",
           "ecosim_data_rich_bassB_lowF",
           "ecosim_data_rich_effort_lowF",
           "ecosim_data_rich_meanage_lowF")
...
- Project biomass based on $1.5\times F_{MSY}$
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
# Re-run SS3 with Fadj

projection_file_path <- here::here("data", "data_rich", "OM2_projections")
ss3_projection <- list()
fmsy_vec <- aggregate(1.5*value ~ variable, fmsy_data_melt_median, median)
...
for (i in 1:nrow(fmsy_vec)) {
  ...
  file_path = projection_file_path,
  r0 = 12, steepness = 0.55, sigmar = 0.5,
  projection_f = fmsy_vec$`1.5 * value`[i], projection_catch = NULL,
  sa_data = sa_data, model_year = model_year,
  projection_year = projection_year,
  use_depletion = FALSE, depletion_ratio = NULL,
  initial_equilibrium_catch = TRUE, 
...
cases <- c("ecosim_data_rich_ss3_highF",
           "ecosim_data_rich_amo_highF",
           "ecosim_data_rich_pdsi_highF",
           "ecosim_data_rich_bassB_highF",
           "ecosim_data_rich_effort_highF",
           "ecosim_data_rich_meanage_highF")
