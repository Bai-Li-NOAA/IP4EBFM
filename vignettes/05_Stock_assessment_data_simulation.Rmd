...
```{r, results='hide', echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
plot(0:6, as.numeric(survey_selectivity$nad[1, ]), type="l", pch=1, col=1, lty=1,
     xlab = "Age", ylab = "Selectivity")
lines(0:6, as.numeric(survey_selectivity$mad[1, ]), pch=2, col=2, lty=2)
lines(0:6, as.numeric(survey_selectivity$sad[1,]), pch=3, col=3, lty=3)
lines(0:6, as.numeric(survey_selectivity$yoy[1, ]), pch=4, col=4, lty=4)
legend("topright",
...
- When computing observed length composition data, variation in length-at-age is computed first. However, age 0 fish all go to the first length bin. Later, when applying selectivity-at-age using BAM NAD selectivity estimates, all the age 0 fish go to the first length bin, result in a relatively high proportion value at the first length bin. Therefore, slope parameter was modified to a higher value (from 2.3 to 3) to reduce the proportion at the first length bin. Would be good to check the code from line 140 to line 180 in the [create_survey.R file](https://github.com/Bai-Li-NOAA/IFA4EBFM/blob/5f663122facdf749c3c1d8256c8ecb3f533d46c6/R/create_survey.R#L140). 
