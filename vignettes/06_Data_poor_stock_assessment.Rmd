...
ss_case <- ss_case03
ss_case@Dep <- ssd_dep$dep[which.min(ssd_dep$ssd)] # dep = 0.75 has the smallest sum of squared differences: 4.7e+12
dbsra <- DLMtool::DBSRA_(
  x = 1,
  Data = ss_case,
  depo = NULL,
...
ss_case <- ss_case03
ss_case@Dep <- ssd_dep$dep[which.min(ssd_dep$ssd)]
ss_case@BMSY_B0 <- ssd_bmsy_b0$bmsy_b0[which.min(ssd_bmsy_b0$ssd)] # bmsy_b0 = 0.3 has the smallest sum of squared differences: 6.6e+12
dbsra <- DLMtool::DBSRA_(
  x = 1,
  Data = ss_case,
  depo = NULL,
...
ss_case@Dep <- ssd_dep$dep[which.min(ssd_dep$ssd)]
ss_case@BMSY_B0 <- ssd_bmsy_b0$bmsy_b0[which.min(ssd_bmsy_b0$ssd)]
ss_case@FMSY_M <- ssd_fmsy_m$fmsy_m[which.min(ssd_fmsy_m$ssd)] # fmsy_m = 0.8 has the smallest sum of squared differences: 4.9e+12
dbsra <- DLMtool::DBSRA_(
  x = 1,
  Data = ss_case,
  depo = NULL,
...
ggplot(soi_data_melt[soi_data_melt$projection_year_id==length(projection_year), ], aes(x = year, y = value)) +
  geom_line() +
