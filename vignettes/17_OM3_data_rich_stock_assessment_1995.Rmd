...
biomass_f0 <- data.frame(
  ...
  biomass_f0_menhaden = biomass_f0_menhaden, 
  biomass_f0_menhaden_bass = biomass_f0_menhaden_bass, 
  biomass_f0_all = biomass_f0_all,
  biomass_f0_all_noEnv = biomass_f0_all_noEnv
)

abundance0_f0 <- data.frame(
  year = start_year:(start_year + length(time_id) - 1),
  data_type = "Abundance of age 0 menhaden-like species",
  abundance0_f0_menhaden = abundance0_f0_menhaden, 
  abundance0_f0_menhaden_bass = abundance0_f0_menhaden_bass, 
  abundance0_f0_all = abundance0_f0_all,
  abundance0_f0_all_noEnv = abundance0_f0_all_noEnv
)
...
ggplot(rbind(biomass_f0_reshape, abundance0_f0_reshape), aes(x = year, y = value, color = variable)) +
  geom_line() +
  facet_wrap(~data_type) +
  labs(
    title = "Unfished condition",
    x = "Year",
    y = "Value"
  ) +
  theme_bw()
...
yrange <- range(biomass_f0_menhaden, biomass_f0_menhaden_bass, 
                biomass_f0_all, biomass_f0_all_noEnv)
plot(x, biomass_f0_menhaden,
  xlab = "Year", ylab = "Biomass of menhaden-like species (mt)",
  lty = 1, type = "l"
)
...
legend("topright",
  c("Menhaden F = 0", 
    "Menhaden & Bass F = 0",
    "All functional groups F = 0", 
    "All functional groups F = 0 & no environmental impacts"),
  lty = c(1, 2, 3, 4),
  bty = "n"
)

yrange <- range(abundance0_f0_menhaden, abundance0_f0_menhaden_bass, 
                abundance0_f0_all, abundance0_f0_all_noEnv)
plot(x, abundance0_f0_menhaden,
  xlab = "Year", ylab = "Abundance of age 0 menhaden-like species",
  lty = 1, type = "l"
)
lines(x, abundance0_f0_menhaden_bass,
  lty = 2
)
lines(x, abundance0_f0_all,
  lty = 3
)
lines(x, abundance0_f0_all_noEnv,
  lty = 4
)
legend("topright",
  c("Menhaden F = 0", 
...
```{r}
print(ss3list$sigma_R_info$alternative_sigma_R[1])
print(fmsy)
```

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=10}
ci_calculation <- function(distribution, value, std) {
...
ylim <- range(ssb_ewe[time_id], ss3list$timeseries$SpawnBio[1:length(model_year)])
plot(c(model_year),
  ssb_ewe[time_id],
  xlab = "Year", ylab = "SB (mt)",
  ylim = ylim,
  pch = 16
)
lines(model_year, ss3list$timeseries$SpawnBio[model_year_id], lty = 1)

legend("topleft",
  c("EwE", "SS3 Estimation"),
  bty = "n",
  pch = c(16, NA),
  lty = c(NA, 1)
)
...
# Biomass
ylim <- range(biomass_ewe[time_id], ss3list$timeseries$Bio_all[1:length(model_year)])
plot(c(model_year),
  biomass_ewe[time_id],
...
ylim <- range(abundance_ewe[time_id], ss3list$timeseries$`SmryNum_SX:1_GP:1`[1:length(model_year)] * 1000)
plot(c(model_year),
  abundance_ewe[time_id],
...
ylim <- range(recruit_ewe[time_id], recruit_ss3$recruit[1:length(model_year)])
plot(c(model_year),
  recruit_ewe[time_id],
...
ylim <- range(mortality_ewe[1:length(model_year)], ss3list$timeseries$`F:_1`[1:length(model_year)])
plot(c(model_year),
  mortality_ewe[1:length(model_year)],
